...
class AbstractAdditionalAllCoursesColumn(object):
    ...
    def produce_table_row(cls, app_context):
        ...
        button_id = "save_admin_email_"+namespace
        a = safe_dom.Element('p').add_child(
                safe_dom.Element('input', id = email_id, name ='admin_email', value = admin_email,
                className = 'admin_email')
            ).add_child( 
                safe_dom.Element('button', id = button_id, className = 'save_admin_email gcb-button', 
                type ='submit', name = namespace
                ).add_text('Save')
            )
...
class BaseAdminHandler(ConfigPropertyEditor):
    ...
    obj1 = AbstractAdditionalAllCoursesColumn()
